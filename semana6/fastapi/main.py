...
app = FastAPI(title = "API Queso Y Sabor - Catalogo", version = "1.0.0", lifespan=lifespan)

class ItemIn(BaseModel):
    nombre: str = Field(min_length = 1, description = "Nombre del queso")
    description: str = Field(default = "", description = "Detalles de maduración o ingredientes")
    precio: float = Field(gt = 0, description = "Precio > 0")
    tags: List[str] = Field(default_factory = list, description = "Ej: ['Maduro', 'Oveja', 'Sin Lactosa']")
    activo: bool = Field(default = True, description = "Define si hay stock para armar un pedido")
...
async def listar_items(
    q: Optional[str] = Query(None, description = "Filtro por nombre nombre de queso que contenga q"),
...
async def obtener_item(item_id: str):
    if not ObjectId.is_valid(item_id):
        raise HTTPException(status_code = 400, detail = "item_id invalido")
